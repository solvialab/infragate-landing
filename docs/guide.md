...
```
dev.infragate.cloud â†’ <VM_PUBLIC_IP>  (A record)
```
...
> Replace `<infragate-group-name>` with your IAM group (for example `InfragateSvcGroup`) and `<your-compartment>` with your compartment name.
>
> If your group is not in the default identity domain, use fully qualified syntax: `group '<identity-domain-name>'/'<infragate-group-name>'`.

Create a second root-level policy for the OKE service itself (one-time per tenancy):

```
Allow service OKE to manage all-resources in tenancy
```

> This service policy is separate from `infragate-svc` user/group policies. If missing, OKE API calls such as `GetNodePoolOptions` can fail with `RelatedResourceNotAuthorizedOrNotFound` / "Unable to retrieve required tenancy details".

Optional alternative policy style (legacy/fine-grained principal targeting):
...
Allow any-user to read tenancies in tenancy where request.user.id = '<infragate-svc-user-ocid>'
Allow any-user to inspect compartments in tenancy where request.user.id = '<infragate-svc-user-ocid>'
Allow any-user to use tag-namespaces in tenancy where request.user.id = '<infragate-svc-user-ocid>'
```
...
Use **Sync from OCI** to fetch OKE-compatible shapes for your current region/tenancy, then optionally refine labels or add custom fallback entries manually with **Add shape**.

If you want to cross-check via CLI, fetch what OKE currently supports:

```bash
oci ce node-pool-options get --node-pool-option-id all --profile <PROFILE>
```

Use the returned `data.shapes` list as the source of truth for what should be enabled in Admin Configuration.

If **Sync from OCI** is unavailable, Infragate keeps current `allowed_shapes` unchanged. This sync call uses backend API pod OCI service account credentials and IAM policies, not the logged-in admin browser profile.
...
Use the returned `data.sources` entries to choose image OCIDs that are compatible with your selected shape and Kubernetes version in that region.
...
Use the OCI-fetched checklist to select which Kubernetes versions users can deploy. Enable/disable/remove actions apply immediately (no separate save button). Use **Refresh from OCI** to pull the latest supported list for your region.

Deploy form behavior: the Kubernetes dropdown is shape-aware. Users only see versions that are compatible with the currently selected VM shape in that region (based on OKE node image availability) and enabled in Admin Configuration. If compatibility lookup is unavailable, the list is fail-closed (not shown) until lookup succeeds. If a version is missing, enable it in Admin Configuration or choose a compatible shape.
